...
use nom::character::complete::{char, alphanumeric1, digit1};
...
pub fn parser_euclidean(input: &str) -> IResult<&str, Euclidean> {
    map_res(
        tuple((parser_value, parser_numbers)),
        |value| Euclidean::create(value.0, value.1.0, value.1.1, value.1.2)
    )(input)
}
...
fn parser_numbers_inner(input: &str) -> IResult<&str, (EuclideanPrimitive, EuclideanPrimitive, Option<EuclideanPrimitive>)> {
